...
static struct {
    ...
    pb_far_t        ver_far;
    int             nenrolled;
    int             enrolled_ids[MAX_FINGERS];
    pb_template_t*  enrolled_templates[MAX_FINGERS];
} Global;

static pb_rc_t ui_display_event (pb_session_t* session,
                                 int event_,
                                 pb_finger_t* finger)
{
    switch(event_) {
...
static pb_rc_t ui_display_synthetic_image (pb_session_t* session,
                                           const pb_image_t* image)
{
    return PB_RC_NOT_SUPPORTED;
}
static pb_rc_t ui_display_islands (pb_session_t* session,
                                   pb_mte_island_t* islands,
                                   uint8_t nbr_of_islands)
{
    return PB_RC_NOT_SUPPORTED;
}
static pb_rc_t ui_display_quality (pb_session_t* session,
...
static pb_rc_t ui_display_progress (pb_session_t* session,
                                    uint16_t completed,
                                    uint16_t total)
{
    return PB_RC_NOT_SUPPORTED;
...
static pb_image_t* capture_image(int ix, int enroll) // params only for demo code
{
    return pb_image_create(192, 192, 508, 508, 0, PB_IMPRESSION_TYPE_LIVE_SCAN_PLAIN);
}
extern "C"
JNIEXPORT int JNICALL
Java_com_senvis_finger_senvisdemo_SenvisService_test(
    JNIEnv *env,
    jobject /* this */)
{/*
    pb_user_t* user;
    ...
    rev = pb_prod_rev();
    __android_log_print(ANDROID_LOG_INFO, "ShinJAE", "Version %s (%d.%d.%d.%d)\n", pb_prod_revision(),
           PB_PROD_MAJOR(rev), PB_PROD_MINOR(rev),
           PB_PROD_RELEA(rev), PB_PROD_PATCH(rev));

    //if (argc > 1) id = strtoul(argv[1], 0, 0);
    user = pb_user_create(id);
    __android_log_print(ANDROID_LOG_INFO, "ShinJAE", "Hello BMF user %d\n", pb_user_get_id(user));
    pb_user_delete(user);
*/


    pb_multitemplate_enroll_t* mte = 0;
    ...
    mte = pb_multitemplate_enroll_create_algorithm(Global.algorithm,
                                                   PB_FINGER_ANONYMOUS,
                                                   &UiFeedbackHandler,
                                                   0);
    if (!mte) return 1;

    max_samples = pb_algorithm_get_config(Global.algorithm)->max_nbr_of_enrollment_templates;

    for (i = num_accepted = 0; num_accepted < max_samples; i++) {
        pb_image_t* image;

        /* Need some way to exit enrollment if we never get enough
         * accepted enrollment samples, in a real system this can
         * be a timeout or user inactivity. */
        if (i > 2 * max_samples) { i--; break; }

        image = capture_image(0, 1);

        if (image) {
            res = pb_multitemplate_enroll_run(mte, image, 0, &coverage);
            pb_image_delete(image); image = 0;
            num_accepted = pb_multitemplate_enroll_get_nbr_of_captures(mte);
            if (res == PB_RC_CAPACITY) break;
            else if(res != PB_RC_OK) {
                goto errexit;
            }
        }
    }
    res = pb_multitemplate_enroll_finalize_template_algorithm (Global.algorithm, mte, &enrolled_template);
    if(res != PB_RC_OK) {
        goto errexit;
    }

    if (num_accepted < max_samples ) {
        result = 1; // Fail enrollment or have user try again
    } else {
        result = 0;
        // Store template, here simply retain a live reference to the pb_template
        // object, for persistence save the type, size and content.
        pb_template_delete(Global.enrolled_templates[0]);
        Global.enrolled_templates[0] = pb_template_retain(enrolled_template);
        Global.nenrolled = 1;
    }
...
JNIEXPORT int JNICALL
Java_com_senvis_finger_senvisdemo_SenvisService_fin(
    JNIEnv *env,
    jobject /* this */)
{
    flag = false;
...
Java_com_senvis_finger_senvisdemo_SenvisService_stringFromJNI(
    JNIEnv *env,
    jobject /* this */) {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_senvis_finger_senvisdemo_SenvisService_SpiOpen(
    JNIEnv *env,
    jobject,
    int r1,
    int r2,
...
extern "C"
JNIEXPORT int JNICALL
Java_com_senvis_finger_senvisdemo_SenvisService_FPgetTemp2(
    JNIEnv *env,
    jobject obj) {
...
Java_com_senvis_finger_senvisdemo_SenvisService_FPLoop(
    JNIEnv *env,
    jobject obj,
    jobject bitmap) {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_senvis_finger_senvisdemo_SenvisService_PBTest(
        JNIEnv *env,
        jobject,
        int id) {
